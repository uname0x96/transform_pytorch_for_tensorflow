...
_FLOAT_DTYPE = [tf.float16, tf.float32, tf.float64, tf.bfloat16]
...
def _is_tensor_image(img):
    return tf.is_tensor(img) and len(img.shape) == 3
...
def _is_numpy_image(img):
    ...
    #     raise TypeError('img is not ndarray. Got {}.'.format(type(img)))
    #     return False
...
def to_tensor(pic):
    ...
    if isinstance(pic, np.ndarray):
        ...
        if (img.dtype is tf.uint8):
            img = tf.cast(img, dtype=tf.float32)
            return tf.math.divide(img, 255)
        else:
            return img

    # accimage is a partial replacement for PIL.Image. They wrote accimage to work around the bottleneck problem.
    # No need convert this to tensorflow. So maybe will make your training become slower. I'll update this code to TF later.
    ...
    # handle PIL Image
    if pic.mode == 'I':
        img = tf.convert_to_tensor(np.asarray(pic), dtype=tf.int32)
        # img = torch.from_numpy(np.array(pic, np.int32, copy=False))
    elif pic.mode == 'I;16':
        img = tf.convert_to_tensor(np.asarray(pic), dtype=tf.int16)
        # img = torch.from_numpy(np.array(pic, np.int16, copy=False))
    elif pic.mode == 'F':
        img = tf.convert_to_tensor(np.asarray(pic), dtype=tf.float32)
        # img = torch.from_numpy(np.array(pic, np.float32, copy=False))
    elif pic.mode == '1':
        img = tf.convert_to_tensor(np.asarray(pic), dtype=tf.uint8)
        img = tf.multiply(img, 255)
        # img = 255 * torch.from_numpy(np.array(pic, np.uint8, copy=False))
    else:
        # raise TypeError('tf_functional not support bytedata at this time. We will support it later!')
    ...
    img = tf.reshape(img, [pic.size[1], pic.size[0], nchannel])
    # put it from HWC to CHW format
    # yikes, this transpose takes 80% of the loading time/CPU
    img = tf.transpose(img, perm=[2,0,1])
    # img = tf.transpose(img, (0,2))
...
def to_pil_image(pic, mode=None):
    ...
    if not(isinstance(pic, ops.EagerTensor) or isinstance(pic, ops.Tensor) or isinstance(pic, np.ndarray)):
        ...
        if len(pic.shape) not in {2, 3}:
            raise ValueError('pic should be 2/3 dimensional. Got {} dimensions.'.format(len(pic.shape)))
    ...
    npimg = pic
    if (tf.is_tensor(pic) and (pic.dtype in _FLOAT_DTYPE) and mode != 'F'):
        pic = tf.cast(tf.multiply(pic, 255.0), dtype=tf.uint8)
        # pic = pic.mul(255).byte()
    if isinstance(pic, ops.Tensor):
...
def normalize(tensor, mean, std, inplace=False):
    ...
    std = tf.convert_to_tensor(std, dtype=dtype)
    # tensor.sub_(mean[:, None, None]).div_(std[:, None, None])
    tensor = tf.subtract(tensor, mean[:, None, None])
    tensor = tf.divide(tensor, std[:, None, None])
...
def _get_perspective_coeffs(startpoints, endpoints):
    ...
    A = tf.convert_to_tensor(matrix, dtype=tf.float32)
    # A = torch.tensor(matrix, dtype=torch.float)
    B = tf.convert_to_tensor(startpoints, dtype=tf.float32)
    B = tf.reshape(B, shape=[8])
    # B = torch.tensor(startpoints, dtype=torch.float).view(8)
    # print(B.shape, A.shape)
    res = tf.linalg.lstsq(A, B)
    # print(res)
    res = tf.expand_dims(res,axis=1).numpy()
    return res.tolist()
...
def erase(img, i, j, h, w, v, inplace=False):
    ...
    """
    if not isinstance(img, ops.Tensor):
        raise TypeError('img should be Tensor Image. Got {}'.format(type(img)))

    if not inplace:
        # img = img.clone()
        img = tf.identity(img).numpy()
...
if __name__ == "__main__":
    ...
    # print(img_tf.is_floating)
    # print(isinstance(img_tf, ops.Tensor))
    # print(to_pil_image(img_np))
    tensor = img_tf
    mean = [2]
    std = [2]

    # print(normalize(tensor, mean, std, inplace=False))
    # A = [(1,1),(1,2),(1,3),(1,4)]
    # B = [(2,5),(2,6),(2,7),(2,8)]
    # print(_get_perspective_coeffs(B,A))

    print(vflip(img))
    print(five_crop(img, (20,20)))
